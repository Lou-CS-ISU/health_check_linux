...
# 
export AB_HOME;AB_HOME=${AB_HOME:-/usr/local/ab/ai-current-version}
...
unset GDE_EXECUTION
unset ENV
export AB_COMPATIBILITY;AB_COMPATIBILITY=3.3.2.7

# Deployed execution script for graph ccp_generic_history_prepare compiled at 2017-10-28 18:54:44 using Co>Operating System version 3.3.2.7
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/src/developers/u496456/sandboxes/main/sand/ccp_pub}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=ccp_generic_history_prepare

_AB_PROXY_DIR="$(pwd)/"ccp_generic_history_prepare-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/ccp_generic_history_prepare.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/ccp_generic_history_prepare.mp'   -prohibit-legacy-codegen
   mpjret=$?
